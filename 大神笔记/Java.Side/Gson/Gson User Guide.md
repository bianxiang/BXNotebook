...
主要的类是 `Gson`，通过 `new Gson()` 就可以创建。还有一个 `GsonBuilder` 类，可以用来创建 `Gson` 对象，支持一些设置，如版本控制。

执行操作时 `Gson` 对象不会维护任何状态。因此可以重用 `Gson` 对象，用于多个序列化或解序列化。
...
* 使用私有字段没有任何问题（推荐使用！）
* 默认所有字段，包括父类字段包含在序列化中，不需要额外注解。
* 若字段是 `transient`，默认不包含在序列化或解序列化。
* 可以正确处理 null
* 序列化时，null字段不会输出
...
注意集合类型是如何定义的！目前没有办法绕过这个限制。

#### 集合的限制

Gson can serialize collection of arbitrary objects but can not deserialize from it, because there is no way for the user to indicate the type of the resulting object. 解序列化上，集合必须是一个特定的，泛型的类型。

### 序列化与解序列化泛型类型

When you call `toJson(obj)`, Gson calls `obj.getClass()` to get information on the fields to serialize. Similarly, you can typically pass `MyClass.class` object in the `fromJson(json, MyClass.class)` method. This works fine if the object is a non-generic type. 但如果对象是泛型类型，泛型信息由于Java的类型擦除而丢失。Here is an example illustrating the point:
