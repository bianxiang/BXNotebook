...
两种不同的使用方式：通过默认的，或实例化自己的Logger。The former is merely intended to be a convenient shared logger to use throughout your application if you so choose.
...
默认logger默认只配置了Console transport。还可以通过`add()`和`remove()`添加删除transports。
...
Winston supports querying of logs with Loggly-like options. [See Loggly Search API](http://wiki.loggly.com/retrieve_events#optional). Specifically: `File`, `Couchdb`, `Redis`, `Loggly`, `Nssocket`, and `Http`.
...
`winston`能够捕获并打印进程的`uncaughtException`事件。可以通过默认的default winston logger或自定义的logger实例。
...
默认winston在记录uncaughtException后退出。若不想退出，设置`exitOnError = false`
...
There are many transports supported by winston core. If you have a transport you would like to add either open an issue or fork and submit a pull request.
...
* level: 此transport记录的日志级别（默认为'info'）。
* silent：Boolean flag indicating whether to suppress output (default false).
* colorize: Boolean flag indicating if we should colorize output (default false).
* timestamp: 输出前是否要加时间戳。（默认false）。If function is specified, its return value will be used instead of timestamps.
...
* filename: 日志文件的文件名
* maxsize: 日志文件的最大大小。超过后将创建新文件，文件结尾加计数后缀。
