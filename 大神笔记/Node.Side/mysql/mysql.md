...
若想关闭连接，将其从连接池中移除，可以调用 `connection.destroy()`。后续若需要连接时，连接池会创建新的连接。
...
注意，很多MySQL命令会导致隐式的提交，参见[in the MySQL documentation](http://dev.mysql.com/doc/refman/5.5/en/implicit-commit.html)
