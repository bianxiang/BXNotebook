...
每个实例方法都有一个对应的静态方法。例如 `Promise.map(arr, fn)` 等效于  `Promise.resolve(arr).map(fn)`。

## 核心

`Promise` 实例的方法和 `Promise` 类的静态方法。

### `new Promise(Function<Function resolve, Function reject> resolver)` -> `Promise`

创建一个新诺言。传入一个函数，这个函数收到两个参数 `resolve` 和 `reject`。这两个参数决定诺言的命运。

> Node.JS中，很少需要你手工创建诺言，参见：[promisification](#promisification)
...
返回诺言的函数，应保证不会抛出同步的异常。为此常需要在一进入函数时就包裹一个`return new Promise...`。参见 `Promise.try` 和 `Promise.method`。

`resolver`会被{{`Promise` 构造器}}同步调用：
...
[Promises/A+ `.then()`](http://promises-aplus.github.io/promises-spec/). 返回一个新的诺言，链接当前的诺言。The new promise will be rejected or resolved depending on the passed `fulfilledHandler`, `rejectedHandler` and the state of this promise.
...
与`.then`类似，但履行诺言的值或拒绝的原因必须是一个数组，数组会被展平成处理器的参数：
...
若要协调几个独立的并发的诺言，可以使用 `Promise.join()`。

###`.catch(Function handler)` -> `Promise`

注册捕获虽有异常的处理器，等效于`.then(null, handler)`。Any exception happening in a `.then`-chain will propagate to nearest `.catch` handler.

> 为兼容早期ES版本，`.caught()`是`.catch()`的别名。
...
For a parameter to be considered a type of error that you want to filter, you need the constructor to have its `.prototype` property be `instanceof Error`. Such a constructor can be minimally created like so:
...
该方法还支持**基于断言**的过滤器。若传入一个断言函数，而不是某种Error的构造器，断言将接受到错误对象。断言的返回值决定该错误处理器是否被调用。通过断言，可以实现更细粒度的控制。例子：
...
与`.catch`类似，区别是，并不捕获所有异常，只捕获 [operational errors](#operationalerror)。

> 错误，必须是Error，不许是`instanceof Error`的对象，不能是字符串、数字等。参见[a string is not an error](http://www.devthought.com/2011/12/22/a-string-is-not-an-error/)。
...
Pass a handler that will be called regardless of this promise's fate. 返回一个新的诺言，与当前诺言链接。There are special semantics for `.finally()` in that the final value cannot be modified from the handler.

> `.finally()`并不适合做资源管理。参见[resource management](#resource-management)。
...
> 为与早期ES版本兼容，`.lastly()` 是 `.finally()`的别名。
...
任一集合方法都不会修改原输入。数组中的洞当`undefined`值处理。
...
与`.all()`类似，但处理的是对象属性，而不是数组元素。Returns a promise that is fulfilled when all the properties of the object are fulfilled. The promise's fulfillment value is an object with fulfillment values at respective keys to the original object. If any promise in the object rejects, the returned promise is rejected with the rejection reason.
...
You most likely want to use the `.any()` method.
...
管理资源，不让其泄露，比想象中的要难的多。`.finally`是不堪用的，例子：
...
文件读取失败后，`.spread`不会被调用，因此连接不会被关闭。

一种解决办法是，先读文件或先获取连接，一步成功后再作下一步。但这样的“顺序”操作失去了异步操作的优势。{{是吗？}}

利用`using()`，我们既能不泄露资源，又能继续利用并发。

```js
var using = Promise.using;

using(getConnection(), fs.readFileAsync("file.sql", "utf8"),
...
Here is a simple example (where `getConnection()` has been defined to return a proper `Disposer`:
...
However, if the second `getConnection()` throws **synchronously**, the first connection is leaked. This will not happen when using APIs through bluebird promisified methods though. You can wrap functions that could throw in `Promise.method` which will turn synchronous rejections into rejected promises.
...
当一个未处理的拒绝很可能要发生时，bluebird 的默认行为是立即将栈 stack trace 写入日志。自定义错误处理机制可以利用以下钩子：
...
若注册了任何拒绝事件的钩子，不论全局的或局部的，自动日志将被禁用。
