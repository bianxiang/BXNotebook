...
When dealing with TCP streams or the file system, it's necessary to handle octet streams. Node has several strategies for manipulating, creating, and consuming octet streams.

Raw data is stored in instances of the `Buffer` class. A `Buffer` is similar to an array of integers but corresponds to a raw memory allocation **outside** the V8 heap. `Buffer`不能被重设大小。

`Buffer`类是全局的，making it very rare that one would need to ever `require('buffer')`.

Buffer和Javascript字符串对象之间的转换需要编码方法。下面是不同的字符串编码：
...
* `'binary'` - A way of encoding raw binary data into strings by using only the first 8 bits of each character. 这种编码方法已被废止，and should be avoided in favor of `Buffer` objects where possible. This encoding will be removed in future versions of Node.
...
The Buffer class is a global type for dealing with binary data directly. j'n'hIt can be constructed in a variety of ways.
