 ...
 * Simplified 2D drawing via [[SpriteBatch | SpriteBatch, TextureRegions, and Sprite]]. Each actor draws in its own un-rotated and unscaled coordinate system，(0,0)在actor的左下角。
...
`Stage`是一个`InputProcessor`。When it receives input events, it fires them on the appropriate actors. If the stage is being used as a UI on top of other content (eg, a HUD), an `InputMultiplexer` can be used to first give the stage a chance to handle an event. If an actor in the stage handles an event, stage's `InputProcessor` methods will return true, indicating the event has been handled and should not continue on to the next `InputProcessor`.
...
The viewport is specified in the stage constructor or by using `setViewport`. 如果游戏运行在桌面等窗口可以调整大小的平台上，则应用窗口大小改变后，也要改变stage的viewport的大小。

Here is an example of the most basic scene2d application with no actors, using a `ScreenViewport`. 使用`ScreenViewport`，则stage中一个单位对应1个像素。这意味着，stage永远不会被拉伸，但窗口的大小影响着stage的可见区域的大小。This is often useful for UI applications.
...
Passing `true` when updating the viewport changes the camera position so it is centered on the stage, making 0,0 the bottom left corner. This is useful for UIs, 在这种场景下，摄像头的位置一般是不动的。When managing the camera position yourself, pass false or omit the boolean. 如不设置stage位置，默认，0,0在屏幕中央。

Here is an example of using `StretchViewport`. The stage's size of 640x480 will be stretched to the screen size, 可能改变屏幕的长宽比。
